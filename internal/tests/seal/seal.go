...
var created seal.Seal

func Run(t *testing.T, data *data.TestData) seal.Seal {
	testData = data
	add(t)
	update(t)
	list(t)
...
func add(t *testing.T) {
	payload := fmt.Sprintf(`{"serial_number": "seal_%d", "model": 1}`, testData.TimeStamp)

	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/seal", strings.NewReader(payload))
...
func update(t *testing.T) {
	payload := fmt.Sprintf(`{"serial_number": "s_%d"}`, testData.TimeStamp)

	url := fmt.Sprintf("/api/v1/seal/%d", created.Id)
	w := httptest.NewRecorder()
...
func list(t *testing.T) {
	url := fmt.Sprintf(`/api/v1/seal?find=%s`, created.SerialNumber)
	w := httptest.NewRecorder()
