...
var FIND_TYPES = [...]string{"=", "LIKE", "ILIKE", "!=", "IN", ">", ">=", "<", "<=", "is", "is not", "any"}
...
func (q *q[T]) AndWhereExists(expression string) *q[T] {
	return q.writeWhere("AND", fmt.Sprintf("exists (%s)", expression))
}

func (q *q[T]) AndWhereNotExists(expression string) *q[T] {
	return q.writeWhere("AND", fmt.Sprintf("not exists (%s)", expression))
}
...
func (q *q[T]) addWhere(t string, findType uint8, field, value any) *q[T] {
	...
	} else if findType == ANY {
		q.writeWhere(t, fmt.Sprintf("%v = ANY(%v)", value, field))
